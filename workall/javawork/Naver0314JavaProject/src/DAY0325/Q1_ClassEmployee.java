...
public class Q1_ClassEmployee {
	
	public static void showTitle() {
		System.out.println("** 전체 사원정보 출력 **");
	}
	...
	public static void main(String[] args) {
		...
		//1번 데이터 수정
		em[1].setPay(450, 100); //기본급과 수당 같이 수정
		...
		for(int i = 0; i < em.length; i++) {
			System.out.println(i + 1 + "\t" + em[i].getName() + "\t" + em[i].getPosition() + "\t" +
					em[i].getAge() + "\t" + em[i]. getGibon() + "\t" + em[i].getSudang() + "\t" + em[i].getNetPay(em[i].getGibon(), em[i].getSudang()));
		}
